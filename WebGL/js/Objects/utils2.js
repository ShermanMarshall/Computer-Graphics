...
Utils.prototype.rotateww = function(vector1, vector2, angle) {
	...
	var diffX = Math.atan(Math.abs(rotY.y) / Math.abs(rotYx)) - 90;
	
	var rotX = this.rotatex(rotY, diffX, true, true);
	
	var rotated = this.rotatez(rotX, angle, true, true);
	
	var rotX2 = this.rotatex(rotated, -diffX, true, true);

	var rotY2 = this.rotatey(rotX2, -diffY, true, true);

	return new THREE.Vector3(rotY2.x + v1.x, rotY2.y + v1.y, rotY2.z + v1.z);
};

Utils.prototype.translate = function(vector1, vector2) {
	if (vector2 instanceof Object) 
		return {	x: vector1.x + vector2.x, 
				y: vector1.y + vector2.y, 
				z: vector1.z + vector2.z	};
	else
		return {	x: vector1.x + vector2, 
				y: vector1.y + vector2, 
				z: vector1.z + vector2	};	
}

Utils.prototype.rotatephi = function(v1, v2, angle) {
	var toOrigin = this.sum(v1, v2);
};

Utils.prototype.sum = function(v1, v2) {
	return this.toVector(v2.x - v1.x, v2.y - v1.y, v2.z - v1.z);
};

Utils.prototype.toVector = function(x, y, z) {
	return {x: x, y: y, z: z};
};
